...
module('Integration | Component | map', function (hooks) {
  ...
  test('it renders a map for the specified parameters', async function (assert) {
    ...
    const assertMapImage = assert.dom('.map img');
    assertMapImage.exists();
    assertMapImage.hasAttribute(
      'alt',
      'Map image at coordinates 37.7797,-122.4184'
    );
    assertMapImage.hasAttribute('src');
    assertMapImage.hasAttribute('width', '150');
    assertMapImage.hasAttribute('height', '120');
